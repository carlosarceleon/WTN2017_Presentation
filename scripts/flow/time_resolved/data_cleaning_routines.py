...
def read_davis_tecplot_folder_and_rotate_to_serration_surface(
    tecplot_folder,
    plot                = False,
    ...
    df['case_name_2'] = "{0}_a{2}_p{1}_U{3}_z{4}_tr".format(
        device, phi, alpha, U, z
    )
    ...
    if rotate_to_mean_flow:
        ...
        mean_flow_location = 30
        norm_x_05 = find_nearest( mean_flow_location, df.x.values ) 

        df = df.sort_values( by = 'y' )

        mean_flow_angle = atan( 
            df[ ( df.x == norm_x_05 ) & ( df.y < 15 ) \
               & ( df.y > 1 ) ].dropna().v.values.mean() /\
            df[ ( df.x == norm_x_05 ) & ( df.y < 15 ) \
               & ( df.y > 1 ) ].dropna().u.values.mean() 
        )
        mean_flow_angle = degrees( mean_flow_angle )
        if mean_flow_angle:
            print "     Adjusting to mean angle flow by {0:.2f} degrees".\
                    format( mean_flow_angle )
            df = rotate_df( df, mean_flow_angle )
        ...
        mean_flow_rotation_info = pd.DataFrame(
            data = {
                ( df.case_name_2.unique()[0], 'x_loc' ) : mean_flow_location,
                ( df.case_name_2.unique()[0], 'mean_flow_angle' ) : \
                mean_flow_angle,
            }, index = [0]
        )

        if isfile( mean_flow_pickle ):
            mean_flow_rotation_info = pd.concat( 
                [ mean_flow_rotation_info,
                pd.read_pickle( mean_flow_pickle ) ],
                axis = 1
            )

        mean_flow_rotation_info.to_pickle( mean_flow_pickle )




    if rotate_to_mean_flow:
        output_name = "averaged_data/{0}_mean_flow_rotated.p"\
                     .format(df.case_name.unique()[0])
    else:
        output_name = "averaged_data/{0}.p".format(df.case_name.unique()[0])

    if plot:
        show_sample_bls_from_df( 
            df, 
            plot_name = output_name.replace('.p','.png')
        )
...
def show_sample_bls_from_df( df , plot_bl = False, plot_name = '' ):
    ...
    # At 0% 2h:
    if 'STE' in df.case_name.unique()[0] or 'loc10' in df.case_name.unique()[0]:
        pct0 = find_nearest( -2, df.x.values  )
    else:
        pct0 = find_nearest(  2, df.x.values  )
    # At 10% 2h:
    if 'loc00' in df.case_name.unique()[0] \
       and not 'STE' in df.case_name.unique()[0] :
        pct80 = find_nearest( 38, df.x.values )
    elif 'loc05' in df.case_name.unique()[0]:
        pct80 = find_nearest( 18, df.x.values )
    else:
        pct80 = find_nearest( 0, df.x.values )
...
def regrid_df(df,variables = [],resolution=[0]):
    ...
    variables = [v for v in df.columns if not v in string_columns \
...
def show_surface_from_df(df, variable='u', points = [], mask = [],
                        plot_name = '', point_correction = (0, 0) ):
    import matplotlib.pyplot as plt
    from numpy import meshgrid
    ...
    if len(points):
        for p in points:
            px = find_nearest( p[0] - point_correction[0], df.x.unique() )
            py = find_nearest( p[1] - point_correction[1], df.y.unique() )
            axes.scatter( px, py )
...
def show_streamlined_surface_from_df(df, variable='u', 
    ...
    import matplotlib.pyplot as plt
    from numpy import meshgrid,linspace, abs
    from matplotlib import rc
    import seaborn as sns
    ...
    if height_correction or streamwise_correction or angle_correction:
    ...
    df_pivot_v = df.pivot( 'x', 'y', 'v' )
    df_pivot_u = df.pivot( 'x', 'y', 'u' )
    df_pivot_vort = df.pivot( 'x', 'y', 'vorticity_xy' )

    X   = df_pivot_u.index.values
    Y   = df_pivot_u.columns.values
    X,Y = meshgrid(X, Y)
    U   = df_pivot_u.values.T
    V   = df_pivot_v.values.T
    Vort   = df_pivot_vort.values.T
    ...
    if variable == 'flow_angle':
        ...
        cf = axes.contourf(X,Y,U,levels=levels)

    else:
        levels = linspace(0,2000)
    ...
    #    X, Y, rel_U, rel_V, 
    #    density      = 3,
    #    linewidth    = sqrt( rel_U**2 + rel_V**2 )/2.,
    #    color = sqrt( rel_U**2 + rel_V**2 ) / 2.,
    ...
    axes.quiver(
        X[::stride, ::stride * 2],
        Y[::stride, ::stride * 2],
        rel_U[::stride, ::stride * 2],
        rel_V[::stride, ::stride * 2],
        scale=300, color = 'lightgray'
    )
